...
with open('./Outliers.txt', 'w') as f:
	for key in Outliers:
		if key != 'All':
			f.write('%s\n' % key)
			for value in Outliers[key]:
				f.write('%s\n\n' % value)
with open('./All.txt', 'w') as f:
	for value in Outliers['All']:
		f.write('%s\n\n' % value)
...
for key in Outliers:
	if key != 'All':
		dirname = './Examples/' + key
		if not os.path.exists(dirname):
			os.mkdir(dirname)
		for value in Outliers[key]:
			pdb = value[0][-18:-11]
			dirname = './Examples/' + key + '/' + pdb
			if not os.path.exists(dirname):
				os.mkdir(dirname)
			if not os.path.exists(dirname + '/' + value[0][-18:-11] + 'session.pse'):
				folderh = pdb.replace(' ', '_')[0:6].upper()
				folderl = pdb.replace(' ', '_')[0:5].upper() + pdb[6].upper()
				os.system('( cd ' + dirname + ' && python3 ../../../clean_pdb.py ' + folderh.replace('_', ' ') + ' )')
				os.system('( cd ' + dirname + ' && python3 ../../../clean_pdb.py ' + folderl.replace('_', ' ') + ' )')
				heavy_chain = read_fasta_file(dirname +'/' + folderh + '.fasta')[0]
				light_chain = read_fasta_file(dirname +'/' + folderl + '.fasta')[0]
				...
				for counter, pdbfile in enumerate(value[2]):
					print(pdbfile)
					if 'model' in pdbfile:
						pymol.cmd.load(pdbfile + '.pdb', 'AB' + str(counter))
						pymol.cmd.do('run renumber.py')
						pymol.cmd.do('renumber AB' + str(counter) + ' & chain H')
						pymol.cmd.do('renumber AB' + str(counter) + ' & chain L')
					else:
						pymol.cmd.load(pdbfile + '.pdb.gz', 'AB' + str(counter))
					pymol.cmd.color('grey', 'AB' + str(counter))
					if key[-2:] == 'H3':
						pymol.cmd.do('findseq ' +  CDRs['H3'] + ', AB' + str(counter) + ', AB' + str(counter) + 'H3')
						pymol.cmd.color('cyan', 'AB' + str(counter) + 'H3')
					if key[-2:] == 'L3':
						pymol.cmd.do('findseq ' +  CDRs['L3'] + ', AB' + str(counter) + ', AB' + str(counter) + 'L3')
						pymol.cmd.color('cyan', 'AB' + str(counter) + 'L3')
				for counter, pdbfile in enumerate(value[3]):
					if 'model' in pdbfile:
						pymol.cmd.load(pdbfile + '.pdb', 'CM' + str(counter))
						pymol.cmd.do('run renumber.py')
						pymol.cmd.do('renumber CM' + str(counter) + ' & chain H')
						pymol.cmd.do('renumber CM' + str(counter) + ' & chain L')
					else:
						pymol.cmd.load(pdbfile + '.pdb.gz', 'CM' + str(counter))
					pymol.cmd.color('grey', 'CM' + str(counter))
					if key[-2:] == 'H3':
						pymol.cmd.do('findseq ' +  CDRs['H3'] + ', CM' + str(counter) + ', CM' + str(counter) + 'H3')
						pymol.cmd.color('magenta', 'CM' + str(counter) + 'H3')
					if key[-2:] == 'L3':
						pymol.cmd.do('findseq ' +  CDRs['L3'] + ', CM' + str(counter) + ', CM' + str(counter) + 'L3')
						pymol.cmd.color('magenta', 'CM' + str(counter) + 'L3')
				pymol.cmd.load(value[0][0:-11] + '/native.pdb.gz', 'native')
				pymol.cmd.color('grey', 'native')
				pymol.cmd.do('extra_fit all, native')
				pymol.cmd.save(dirname + '/' + value[0][-18:-11] + 'session.pse')
