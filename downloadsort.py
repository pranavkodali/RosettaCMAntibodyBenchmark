...
def Blast(folder, folderall, minscore, maxscore, threshold):
	...
	while text.find('<td class="c1 l lim">', start) > 0: #Look through results file for list of PDBs and score
		...
		dr = ''
		if title[0:4].lower() in antibody_list and title[0:4].lower != folderall[0:4].lower: # Filter by database
			...
			if folder == 'light' and len(score_list[title[0:4].lower()]) == 1:
				score_list[title[0:4].lower()].append(score)
			if minscore < score < maxscore and added < threshold: #Check if score is within range and less than the threshold number of files have been downloaded
				with gzip.open('./templates/' + title[0:4].lower() + '.pdb.gz', 'rb') as f_in:
					with open('./' + folder +'/' + title[0:4].lower() + '.pdb', 'wb') as f_out:
						shutil.copyfileobj(f_in, f_out)
				os.system('python pdb2fasta.py ' + './' + folder +'/' + title[0:4].lower() + '.pdb > '+ './' + folder +'/' + title[0:4].lower() + '.fasta')
				print('python pdb2fasta.py ' + './' + folder +'/' + title[0:4].lower() + '.pdb > '+ './' + folder +'/' + title[0:4].lower() + '.fasta')
				time.sleep(0.05)
				added += 1
		start = idx + 40
	#Run clustering off of grouped files
	p = subprocess.Popen('clustalw.py ' + folder + ' ' + folderall, shell=True)
	q.append(p)
	exitcodes = [p.wait() for p in q]

fragment_list = ['heavy', 'light', 'L1', 'L2', 'L3', 'H1', 'H2', 'H3']
...
for i in fragment_list:
	write_fasta_file(i, CDRs[i], './' + folderall +'/')
	Blast(i, folderall, minimum, maximum, 10)
...
for key in sorted_score_list[:15]:
	with gzip.open('./templates/' + key + '.pdb.gz', 'rb') as f_in:
		with open('./merged/' + key + '.pdb', 'wb') as f_out:
			shutil.copyfileobj(f_in, f_out)
	os.system('python pdb2fasta.py ' + './merged/' + key + '.pdb > '+ './merged/' + key + '.fasta')
	print('python pdb2fasta.py ' + './merged/' + key + '.pdb > '+ './merged/' + key + '.fasta')
	time.sleep(0.05)
q = []
p = subprocess.Popen('clustalw.py merged ' + folderall, shell=True)
q.append(p)
exitcodes = [p.wait() for p in q]
fragment_list.append('merged')

q = []

for name in foldernames:
	fname = os.path.join('./' + folderall, name)
	for root, dirs, files in os.walk(fname):
		for file in files:
			if file.endswith('.pdb'):
				os.system('python pdb2fasta.py ' + fname +'/' + file + ' > '+ fname +'/' + file[0:4] + '.fasta')
	...
	p = subprocess.Popen('python ClustaloGrishin.py ' + fname + '/alignment.aln ' + fname[2:], shell=True)
	q.append(p)


for folder in fragment_list:
	shutil.rmtree(folder)

exitcodes = [p.wait() for p in q]

os.system('python model_control.py ' + sys.argv[1])
os.system('bash -c "source ~/.bashrc && python runrmsd.py ' + sys.argv[1] + ' 0"')
os.system('python runbash.py ' + sys.argv[1] + HOST)
